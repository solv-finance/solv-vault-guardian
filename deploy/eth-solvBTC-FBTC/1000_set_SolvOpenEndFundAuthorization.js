...
module.exports = async ({ getNamedAccounts, deployments }) => {
  ...
  const guardianAddress = (await deployments.get("eth-solvBTC-FBTC-SolvVaultGuardianForSafe13")).address;
  ...
  const openEndFundAuthorizationAddress = (await deployments.get("eth-solvBTC-FBTC-SolvOpenEndFundAuthorization")).address;

  const erc20AuthorizationAddress = (await deployments.get("eth-solvBTC-FBTC-ERC20Authorization")).address;
  ...
  await guardian.setAuthorization(FBTC, erc20AuthorizationAddress);
  console.log(`set authorization ${erc20AuthorizationAddress} for contract ${FBTC}`);
...
module.exports.tags = ["eth-solvBTC-FBTC-set-SolvVaultGuardianForSafe13"];
