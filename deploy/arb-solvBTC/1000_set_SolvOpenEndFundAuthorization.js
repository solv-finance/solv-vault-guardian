...
module.exports = async ({ getNamedAccounts, deployments }) => {
  ...
  const erc20AuthorizationAddress = (
    await deployments.get("arb-solvBTC-ERC20Authorization")
  ).address;
  ...
  const openEndFundAuthorizationAddress = (
    await deployments.get("arb-solvBTC-SolvOpenEndFundAuthorization")
  ).address;
  ...
  const guardianAddress = (
    await deployments.get("arb-solvBTC-SolvVaultGuardianForSafe13")
  ).address;
  const guardian = guardianFactory.attach(guardianAddress);

  await guardian.setAuthorization(WBTC, erc20AuthorizationAddress);
  console.log(
    `set authorization ${erc20AuthorizationAddress} for contract ${WBTC}`
  );
