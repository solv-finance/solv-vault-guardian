...
abstract contract BaseACL {
	...
	function preCheck(bytes calldata data_) external virtual onlyCaller returns (Type.CheckResult memory result_) {
		result_ = _preCheck(data_);
	}

    function _preCheck( bytes calldata data_
    ) internal virtual returns (Type.CheckResult memory result_) {
		(bool success, bytes memory revertData) = address(this).staticcall(data_);
    ...
    function _unpackTxn() internal pure virtual returns (Type.TxData memory txData_) {
    ...
    function _txn() internal pure virtual returns (Type.TxData memory) {
        return _unpackTxn();
    }
}